...
public:
  ...
  BOOL Init(LPCSTR szSymPath)
  {
	...
	pMiniDumpWriteDump = (tMiniDumpWriteDump) GetProcAddress(m_hDbhHelp, "MiniDumpWriteDump" );

    if ( pSC == NULL || pSFTA == NULL || pSGMB == NULL || pSGMI == NULL ||
      pSGO == NULL || pSGSFA == NULL || pSI == NULL || pSSO == NULL ||
      pSW == NULL || pUDSN == NULL || pSLM == NULL || pMiniDumpWriteDump == NULL)
    {
      FreeLibrary(m_hDbhHelp);
      m_hDbhHelp = NULL;
      pSC = NULL;
...
//  tSGMI_V3 pSGMI_V3;
  ...
  typedef BOOL (__stdcall WINAPI *tMiniDumpWriteDump) (
	  HANDLE hProcess,
	  DWORD ProcessId,
	  HANDLE hFile,
...
BOOL StackWalker::GenerateMiniDump(LPCSTR szFileName, _EXCEPTION_POINTERS* exceptionInfo, std::string& errorMsg)
{
	if (m_modulesLoaded == FALSE)
		this->LoadModules();
	...
	{
		SetLastError(ERROR_DLL_INIT_FAILED);
		errorMsg = "DLL Init Failed";
		return FALSE;
	}

	if (this->m_sw->pMiniDumpWriteDump == NULL || szFileName == NULL)
	{
		errorMsg = "MiniDumpWriteDump not found";
		return FALSE;
	}
	...
	MINIDUMP_EXCEPTION_INFORMATION *pExpInfo = (exceptionInfo? &expInfo: 0);

	if (!this->m_sw->pMiniDumpWriteDump(
			this->m_hProcess, 
			this->m_dwProcessId,  
			file, 
			MiniDumpNormal, 
			...
			NULL))
	{
		...
		os << "MiniDumpWriteDump failed with error code : " << GetLastError();
		errorMsg = os.str();
		return FALSE;
	}
  ...
  for (frameNum = 0; ; ++frameNum )
  {
    ...
    if (s.AddrPC.Offset != 0)
    {
      ...
      if (this->m_sw->pSGSFA(this->m_hProcess, s.AddrPC.Offset, &(csEntry.offsetFromSmybol), pSym) != FALSE)
      {
		...
		strncpy_s(csEntry.name, pSym->Name, STACKWALK_MAX_NAMELEN - 2);
        // UnDecorateSymbolName()
...
{
  CHAR buffer[STACKWALK_MAX_NAMELEN * 2];
  if ( (eType != lastEntry) && (entry.offset != 0) )
  {
    ...
    if (entry.lineFileName[0] == 0)
    {
      ...
      _snprintf_s(buffer, STACKWALK_MAX_NAMELEN * 2, "%p (%s): %s: %s\n", (LPVOID) entry.offset, entry.moduleName, entry.lineFileName, entry.name);
    }
    else
      _snprintf_s(buffer, STACKWALK_MAX_NAMELEN * 2, "%s (%d): %s\n", entry.lineFileName, entry.lineNumber, entry.name);
    OnOutput(buffer);

	m_stackString += buffer;
	//m_stackString += "\n";
  }
}
