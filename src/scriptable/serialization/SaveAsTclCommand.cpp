...
#include "SaveAsTclCommand_AutoGen.cpp"
#include "StcSystem.h"
...
CSaveAsTclCommand::CSaveAsTclCommand() {
	SetUserCanInterrupt(false);
    SetRunnableName("Save As Tcl");
}
CSaveAsTclCommand::~CSaveAsTclCommand() {}

void CSaveAsTclCommand::DoRun() 
{
    ...
    Saver::saveAsScript(*root, filename, GetSaveDefault(), 
                        GetProcedurePrefix(), GetBeforeDisconnectScript(),
                        GetTclNamespace(), filePathMap, TCL, GetUseDefaultsForRarelyUsedSettings());
