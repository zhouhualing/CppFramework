...
public:
	...
	{
		stc::framework::CScriptable::Set(0x00018001, val);
	}
	/// \brief Returns the TestSessionDirectory
	inline std::string GetTestSessionDirectory() const
	{
		return CScriptable::GetString(0x00018001);
	}

	/// \brief Sets the FileCategoryFilter
	inline void SetFileCategoryFilter(const uint32_t val)
	{
		stc::framework::CScriptable::Set(0x00018002, val);
	}
	/// \brief Returns the FileCategoryFilter
	inline uint32_t GetFileCategoryFilter() const
	{
		return CScriptable::GetU32(0x00018002);
	}
